...
class NotEquallTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $testData = json_decode(file_get_contents(__DIR__ . '/../../../../../mystique/testdata/notEqual.json'), true);
        ...
        foreach ($testData['data'] as $testPoint) {
            ...
            if (isset($compare['day']) && isset($compare['month']) && isset($compare['year'])) {
                $testPoint['options']['compare'] =
                    new \DateTime($compare['year'] . '-' . $compare['month'] . '-' . $compare['day']);
            }
